...
public class PersonController {
    ...
    public ResponseEntity<PersonCreate> addPersonOnly(@RequestBody @Valid PersonCreate personDTO) {

        PersonCreate saved = personService.personAdd(personDTO);
        
        return ResponseEntity.status(HttpStatus.CREATED).body(saved);
    }

    @PostMapping(value = "/withPets", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<PersonCreate> addPersonAndPets(
            @RequestPart("personWithPetsDTO") PersonWithPetsDTO dto,
            @RequestPart("image") @Valid MultipartFile image) {

        PersonCreate saved = personService.addPersonWithPets(dto, image);
